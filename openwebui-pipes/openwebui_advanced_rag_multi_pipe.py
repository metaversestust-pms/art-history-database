"""
title: Advanced RAG（多模型）
author: Art History Database Team
version: 1.0.0
description: 多級檢索與重排序（Advanced RAG），可切換 6 種 LLM 模型
"""
...
class Pipe:
    class Valves(BaseModel):
        ...
        TIMEOUT: int = Field(default=320, description="請求逾時秒數（含大型模型冷啟動時間）")
        SHOW_SOURCES: bool = Field(default=True, description="是否在回答後附上參考來源")

    def __init__(self):
        self.id = "advanced_rag_multi"
        self.name = ""
        self.valves = self.Valves()

    def pipes(self):
        return [
            {"id": sub_id, "name": f"{llm_name} + Advanced RAG"}
