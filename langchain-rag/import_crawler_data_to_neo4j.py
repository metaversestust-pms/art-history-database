                    ...
                    for item in data.get('records', []):
                        ...
                        if item.get('people'):
                            for person in item['people']:
                                if person.get('role') in ['Artist', 'Creator'] and person.get('name'):
                                    artist_query = """
                                    MERGE (artist:Artist {name: $name})
...
def main():
    ...
    try:
        ...
        print(f"\n🎉 Neo4j數據導入完成！")
        print(f"   您的知識圖譜現在包含完整的爬蟲數據")
        print(f"   可以通過GraphRAG進行複雜的關係查詢")
