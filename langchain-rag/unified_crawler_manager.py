...
class UnifiedCrawlerManager:
    ...
    def crawl_harvard_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"✅ Harvard Art Museums數據爬取完成")
        ...
        try:
            ...
            if 'objects' in harvard_data and harvard_data['objects'].get('records'):
                ...
                for obj in objects:
                    ...
                    entities, relationships = self.data_mapper.map_single_item(standardized_obj, source="harvard")
            ...
            if 'people' in harvard_data and harvard_data['people'].get('records'):
                ...
                for person in people:
                    ...
                    entities, relationships = self.data_mapper.map_single_item(standardized_person, source="harvard")
                    mapped_entities.extend(entities)
                    mapped_relationships.extend(relationships)
            ...
            if 'exhibitions' in harvard_data and harvard_data['exhibitions'].get('records'):
                ...
                for exhibition in exhibitions:
                    ...
                    entities, relationships = self.data_mapper.map_single_item(standardized_exhibition, source="harvard")
                    mapped_entities.extend(entities)
                    mapped_relationships.extend(relationships)
            ...
            logger.info(f"✅ Harvard數據映射完成: {len(unique_entities)} 個實體, {len(mapped_relationships)} 個關係")
            return {
                "status": "success",
        ...
        try:
            ...
            if self.config.harvard_enabled:
                ...
                if harvard_result['status'] == 'success':
                    ...
                    if mapped_result['status'] == 'success':
                        ...
                        crawl_summary['errors'].append(f"Harvard mapping error: {mapped_result.get('error', 'Unknown')}")
                else:
                    crawl_summary['errors'].append(f"Harvard crawl error: {harvard_result.get('error', 'Unknown')}")
            ...
            if self.config.europeana_enabled:
                ...
                if europeana_result['status'] == 'success':
                    # 映射Europeana數據
                    entities, relationships = self.data_mapper.map_europeana_data(europeana_result['data'])
                    all_entities.extend(entities)
                    all_relationships.extend(relationships)
                    crawl_summary['sources_processed'].append('Europeana')
                else:
                    crawl_summary['errors'].append(f"Europeana error: {europeana_result.get('error', 'Unknown')}")
