...
def check_nvidia_gpu() -> Dict[str, Any]:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return {'available': True, 'gpu_info': 'GPU detected', 'nvidia_smi_output': result.stdout}
        else:
    ...
    for package in packages:
        try:
            if package == 'sentence_transformers':
                ...
                import faiss
                packages[package] = faiss.__version__ if hasattr(faiss, '__version__') else 'installed'
            elif package == 'numpy':
                import numpy
...
def assess_cuda_readiness() -> Dict[str, Any]:
    ...
    return {
        ...
        'recommendations': generate_recommendations(gpu_check, cuda_check, pytorch_check, packages_check)
    }
...
def generate_recommendations(gpu_check, cuda_check, pytorch_check, packages_check) -> list:
    ...
    if not pytorch_check.get('installed', False):
        recommendations.append("🔥 PyTorch未安裝。運行: source cuda_art_env/bin/activate && pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu121")

    if pytorch_check.get('installed', False) and not pytorch_check.get('cuda_available', False):
        recommendations.append("⚠️ PyTorch已安裝但CUDA不可用。可能需要重新安裝支持CUDA的PyTorch版本。")

    if not packages_check.get('sentence_transformers', False):
        recommendations.append("📚 缺少sentence-transformers。運行: pip install sentence-transformers")
