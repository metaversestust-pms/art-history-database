...
from mcp.mcp_tool_registry import MCPToolRegistry, get_mcp_registry, MCPToolStatus, MCPToolType
    ...
    async def execute(self, request: ProxyRequest) -> ProxyResponse:
        ...
        while retry_count <= request.retry_count:
            try:
                ...
                if retry_count < request.retry_count:
                    retry_count += 1
                    self.logger.warning(f"請求異常，重試 {retry_count}/{request.retry_count}: {str(e)}")
                    await asyncio.sleep(1)
                    continue
                else:
                    self.update_stats(execution_time, False)
...
class LLMToolProxy(HTTPToolProxy):
    ...
    async def chat_completion(self, messages: List[Dict[str, str]],
                            model: str = None, **kwargs) -> ProxyResponse:
        """聊天完成請求"""
        params = {
            "messages": messages,
            "model": model,
    ...
    async def embeddings(self, text: Union[str, List[str]],
                        model: str = None) -> ProxyResponse:
        """文本嵌入請求"""
        params = {
            "input": text,
            "model": model
...
class VectorDBToolProxy(HTTPToolProxy):
    ...
    async def create_collection(self, collection_name: str,
                               dimension: int = None, **kwargs) -> ProxyResponse:
        """創建集合"""
        params = {
    ...
    async def insert_vectors(self, collection_name: str, vectors: List[List[float]],
                           ids: List[str] = None, metadata: List[Dict] = None) -> ProxyResponse:
        """插入向量"""
        params = {
            "collection_name": collection_name,
            "vectors": vectors,
    ...
    async def search_vectors(self, collection_name: str, query_vector: List[float],
                           top_k: int = 10, **kwargs) -> ProxyResponse:
        """向量搜索"""
        params = {
            "collection_name": collection_name,
...
class MultimodalToolProxy(HTTPToolProxy):
    ...
    async def process_image(self, image_data: bytes, task: str = "caption",
                          **kwargs) -> ProxyResponse:
        """處理圖像"""
        # 這裡需要處理圖像數據的編碼
        import base64
    ...
    async def process_audio(self, audio_data: bytes, task: str = "transcribe",
                          **kwargs) -> ProxyResponse:
        """處理音頻"""
        import base64
...
class MCPProxyManager:
    ...
    async def execute_tool_request(self, tool_name: str, method: str,
    ...
    async def broadcast_request(self, tool_type: MCPToolType, method: str,
                              params: Dict[str, Any] = None) -> Dict[str, ProxyResponse]:
        """向同類型的所有工具廣播請求"""
