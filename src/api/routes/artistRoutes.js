...
router.get(
    ...
    artistController.getArtistsByNationality.bind(artistController)
);
...
router.post(
    '/',
    artistValidationMiddleware(false),
    artistController.createArtist.bind(artistController)
);
router.put(
    '/:id',
    artistValidationMiddleware(true),
...
router.post(
    '/bulk',
    bulkArtistValidationMiddleware(),
    artistController.bulkCreateArtists
        ? artistController.bulkCreateArtists.bind(artistController)
        : (req, res) => {
              res.status(501).json({
                  success: false,
                  message: '批量創建功能尚未實現',
