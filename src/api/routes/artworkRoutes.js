...
router.get(
    ...
    artworkController.searchArtworks.bind(artworkController)
);
...
router.post(
    '/',
    artworkValidationMiddleware(false),
    artworkController.createArtwork.bind(artworkController)
);
router.put(
    '/:id',
    artworkValidationMiddleware(true),
    artworkController.updateArtwork.bind(artworkController)
);
...
router.post(
    '/bulk',
    bulkArtworkValidationMiddleware(),
    artworkController.bulkCreateArtworks
        ? artworkController.bulkCreateArtworks.bind(artworkController)
        : (req, res) => {
              res.status(501).json({
                  success: false,
                  message: '批量創建功能尚未實現',
