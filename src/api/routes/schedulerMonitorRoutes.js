    ...
    setupRoutes() {
                    ...
                    data: {
                        ...
                        system: {
                            ...
                            memoryUsage: Math.round(
                                monitoringStatus.systemMetrics.memoryUsage / 1024 / 1024
                            ),
                            memoryTotal: Math.round(
                                monitoringStatus.systemMetrics.memoryTotal / 1024 / 1024
                            ),
                        ...
                        health: {
                            ...
                            criticalAlerts: monitoringStatus.recentAlerts.filter(
        ...
        router.get(
            ...
            asyncWrapper(async (req, res) => {
                ...
                for (const [jobId, jobMetrics] of this.schedulerMonitor.metrics.jobMetrics) {
                    ...
                    if (healthThreshold && jobMetrics.healthScore < parseFloat(healthThreshold))
                        continue;
        ...
        router.get(
            ...
            asyncWrapper(async (req, res) => {
                ...
                const performanceReport = this.schedulerMonitor.getPerformanceReport(
                    parseInt(timeRange)
                );
        ...
        router.get(
            ...
            asyncWrapper(async (req, res) => {
                ...
                // HTTP狀態碼
                const httpStatus =
                    overallStatus === 'critical' ? 503 : overallStatus === 'warning' ? 200 : 200;
                ...
                const results = Array.from(taskTypeStats.values()).map((stats) => ({
                    ...
                    successRate:
                        stats.totalExecutions > 0
                            ...
                            : 1,
                    averageHealthScore:
                        stats.totalJobs > 0 ? stats.averageHealthScore / stats.totalJobs : 0,
                    averageExecutionTime:
                        stats.totalJobs > 0 ? stats.averageExecutionTime / stats.totalJobs : 0
                }));
        ...
        router.get(
            ...
            asyncWrapper(async (req, res) => {
                ...
                // 計算成功率
                trends.successRate =
                    trends.totalExecutions > 0
                        ? trends.successfulExecutions / trends.totalExecutions
                        : 1;
        ...
        router.put(
            ...
            asyncWrapper(async (req, res) => {
                ...
                try {
                    ...
                    if (alertThresholds) {
                        Object.assign(
                            this.schedulerMonitor.options.alertThresholds,
                            alertThresholds
                        );
                    }
            ...
            asyncWrapper(async (req, res) => {
                ...
                try {
                    const exportData = {
                        ...
                        performance: this.schedulerMonitor.getPerformanceReport(
