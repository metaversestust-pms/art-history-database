...
async function testMLAPI() {
    ...
    const testCases = [
        ...
        },
        {
            ...
            method: 'POST',
            data: {
                texts: ['巴洛克風格的建築作品', '印象派的風景畫', '現代抽象藝術作品']
            }
        },
        {
            name: '6. 資料預處理',
    ...
    for (const testCase of testCases) {
        try {
            ...
            console.log(
                `   ✅ 成功 - 狀態碼: ${response.status}, 響應時間: ${result.responseTime}`
            );
            ...
            if (testCase.endpoint === '/ml/models/status' && response.data.models) {
                console.log(
                    `   🤖 載入的模型數量: ${Object.keys(response.data.models || {}).length}`
                );
            }

            if (testCase.endpoint === '/ml/inference' && response.data.results) {
                console.log(
                    `   🧠 推理結果數量: ${Array.isArray(response.data.results) ? response.data.results.length : 1}`
                );
            }
    ...
    const endpointIntegrationResults = {
        ...
        ML路由配置: results.find((r) => r.name.includes('健康檢查') && r.statusCode === 503)
            ? '✅ 已整合'
            : '❌ 未整合',
        'CUDA ML服務': results.some((r) => r.status === 'SUCCESS' && r.name.includes('健康檢查'))
            ? '✅ 運行中'
            : '⚠️ 尚未部署 (預期狀態)',
        API路由功能:
            results.filter((r) => r.statusCode && r.statusCode !== 503).length > 0
                ? '✅ 功能正常'
                : '❌ 路由問題'
    };
