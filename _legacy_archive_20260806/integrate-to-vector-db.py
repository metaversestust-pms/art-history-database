...
class VectorDBIntegrator:
    ...
    def connect_services(self):
        ...
        # 連接 ChromaDB
        try:
            logger.info(f"連接 ChromaDB: {self.chromadb_host}:{self.chromadb_port}")
            ...
            logger.info(f"✅ 集合 'art_history' 準備就緒 (當前文檔數: {self.collection.count()})")
        ...
        try:
            ...
            logger.warning(f"⚠️ Ollama 連接失敗: {e}，將使用預先生成的向量")
    ...
    def get_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"向量生成失敗: {response.status_code}")
                return None
    ...
    def add_to_chromadb(self, artworks: List[Dict[str, Any]]):
        ...
        try:
            ...
            for artwork in artworks:
                ...
                # 準備元資料（只保留字串和數字）
                metadata = {
                    'title': str(artwork['title'])[:500],  # 限制長度
                ...
                # 生成唯一ID
                unique_id = f"{artwork['source']}_{artwork['id']}".replace('/', '_')[:64]
            ...
            with self.neo4j_driver.session() as session:
                for artwork in artworks:
                    ...
                    """, **{
                        ...
                        'description': artwork['description'][:1000]  # 限制長度
                    })
    ...
    def process_data_file(self, file_path: Path):
        ...
        try:
            ...
            # 標準化資料
            source = file_path.stem  # 使用文件名作為來源
            artworks = []
