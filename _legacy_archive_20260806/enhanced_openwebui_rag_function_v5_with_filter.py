"""
title: 藝術史 RAG+LLM 完整智能組合系統 v4.1 - 多資料庫整合 (含問題範圍限制)
...
class Tools:
    def __init__(self):
        ...
        self.rag_strategies = {
            "enhanced_rag": {
                ...
                "use_multidb_server": True,  # 使用多資料庫服務器
    ...
    def detect_model_from_request(self, body: dict) -> str:
        ...
        # 默認使用 llama3.1 + Enhanced RAG（新默認！）
        return "llama3-1-8b-enhanced_rag"
...
def generate_welcome_message() -> str:
    """生成歡迎消息 v4.0"""
    return """🎨 **藝術史RAG+LLM完整智能組合系統 v4.0**

## 🆕 新功能亮點
- **🚀 Enhanced RAG**: 增強型檢索，向量+全文+圖譜混合，支持重排序
- **⚡ 雙服務器支持**: 自動選擇最佳服務器
- **📊 260+藝術家、550+作品向量嵌入**: 17.3%覆蓋率
...
## 🔍 支援的RAG策略 (8種)
- **🚀 Enhanced RAG** ⭐NEW⭐: 增強型混合檢索+重排序
- **⚖️ Hybrid RAG**: 平衡混合策略
- **🔍 Vector RAG**: 純向量語義檢索
- **🕸️ Graph RAG**: 知識圖譜關係檢索
...
**推薦組合**:
- 🚀 Llama 3.1 8B + Enhanced RAG (新默認！)
- 🎯 DeepSeek-R1 8B + Advanced RAG (深度分析)
- ⚡ Gemma3 1B + Naive RAG (極速響應)
...
- 需要增強服務器: {'是' if combo_info.get('use_enhanced_server') else '否'}

請確認：
1. RAG服務器正在運行（端口 8008 或 8009）
2. 網絡連接正常
...
### 📊 **執行信息** {'🆕' if combo_info.get('is_new') else ''}
